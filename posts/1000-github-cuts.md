...
1. **Allow anyone to add labels to issues.** t the very least, allow the person who
...
10. **Remember my selection for the "sort" setting.** I'd love to have
...
12. **Dim down the reaction emojis.** Maybe this is too late, but reaction
    emojis are way too unprofessional for some projects.

## Pull requests

13. **Add a button that automatically merges a pull request as soon as all the
    CI checks pass.** Any additional commits pushed to the branch in the
    interim would cancel it, and it could also be canceled by someone else
    with push access or the PR author.

14. **Add some way to disable the automatic hiding of large diffs.** This
...
15. **Include all issue/PR authors in the authors search.** Right now it only
    lists people with push access. But I generally can't remember people's
    GitHub usernames, and autofilling based on all authors would be very
    helpful.

16. **Better contextual guesses for issue autofilling (after typing `#`).**
...
17. **Auto-update edited comments.** Context: we use a
...
18. **Don't hide full commit messages by default in the commits view.** It
    would be better to encourage people to write good commit messages.

19. **Make issue cross-references in pull request titles work.** I'd rather
...
20. **Allow me to comment on lines that aren't visible by default.** That is,
...
21. **Copying code including lines that aren't visible by default includes an
    extra space to the left for those lines.** This is a straight up bug.
    Probably fixing the previous point would also fix this :)

22. **Make searches include text from the pull request diff.**

23. **When a diff indents a line color the whitespace to the left of the
    line.** (see
    [this](https://twitter.com/asmeurer/status/740611970714480640))

24. **Pull requests can show commits that are already in master.** For
...
25. **Make reviews non-deferred by default.** This is the biggest thing. If I
...
26. **Make Cmd-Enter default to immediate comment.** Barring the above change,
...
27. **Allow reviewing your own pull request.** There's no reason to disallow
    this, and it would often be quite useful to, for instance, mark a work in
    progress PR as such with a "request changes" review. Obviously they would
    be excluded from any required reviews.

28. **Unhide the reviews box.** It should just be the same box as the comment
    box, unstead of buried on the diff tab (see my blog post).

29. **Show review status in the pull request list as a red X or green check.**
    This would make it easier to see which pull requests have reviews.

30. **Allow new commits to invalidate reviews.** That way they work the same
...
31. **Allow requiring zero negative reviews to merge (but not necessarily any
    ...
    unnecessary extra work. OTOH allowing people with push access to block a
    merge with a negative review would be very useful.

## Web editor

32. **The web editor seems to have a search function, but I can't get it to
...
33. **Add basic syntax testing in the web editor for common languages to catch
    basic mistakes.**

## Mobile site

34. **Please make the mobile site work with iOS 10.** I don't see any reason
    why simple things like buttons (like the merge button or the comment
    button) shouldn't work on a slightly older browser.

35. **Add a way to disable the mobile site permanently.** For the most part,
    ...
    every time I visit github.com on my phone it goes to the desktop site.
...
36. **Cannot search issues on mobile.**

37. **Cannot make a line comment that *isn't* a review on mobile.**

38. **Cannot view lines beyond the default diff in pull requests on mobile.**

39. **Show more than 2 lines of the README and 0 lines of the code by
    default.** Yes mobile screens are small but it's also not hard to scroll
    on them.

40. **Support Jupyter notebook rendering on mobile.**

## Files view

41. **GitHub needs a better default color theme for syntax highlighting.**
...
42. **Add MathJax support to Markdown files.** This would be amazingly useful
...
43. **The wikis used to support LaTeX math with MathJax. It would be great if
    this were re-added.**

44. **The ability to set push permissions for the wiki separately from the
    ...
    release notes on our wiki. It works quite well, but the only way it can
    push to the wiki is if we give it push access to the full repo, which is a
    security hole.

## Notification emails

45. **Don't clobber special emails/email headers.** GitHub adds special emails
    ...
    words, as soon as someone @mentions me in one of my issues, I become
    *less* likely to see it, because it no longer gets my label (I get
    @mentioned on a lot of issues and don't have the ability to read all of my
    notification emails).

46. **Readd the "view issue" links in Gmail.** (I forgot what these are
...
47. **Make the requests in the API docs actually return what they show in the
...
48. **Better GraphQL libraries.** This doesn't really count as a "cut". The
    new [GraphQL API v4](https://developer.github.com/v4/) looks interesting,
    but unfortunately, without good libraries to manipulate GraphQL (in my
    case, for Python), it's way too complicated to use in practice. This is a
    request to anyone out there, not just GitHub: write a good GraphQL library
    for Python.

49. **Allow giving deploy key access to just one branch.** That way I can have
    a deploy key for `gh-pages` and minimize the attack surface that the
    existence of that key produces.
...
50. [**Moar themes.**](https://twitter.com/asmeurer/status/831962312122761216)
...
51. **The steps to add HTTPS to an existing GitHub pages custom domain are a
    bit confusing.**. This took us a while to figure out. To get things to
    work, you have to trigger GitHub to issue a cert for the domain. But the
    UI to issue the cert is to paste the domain into the box. So if the domain
    is already there but it doesn't work, you have to re-enter it. Also if you
    want both www and the apex domain to be HTTPS you have to enter them both
    in the box to trigger GitHub to issue a cert. This is primarily a UX
    issue. See
    https://github.com/sympy/sympy.github.com/issues/105#issuecomment-415899934.

## Settings

52. **Automatically protected branches make the branch difficult to delete
...
53. **Add a way to disable the ability for non-admins to create new branches
    on a repo.** We want all of our branches to come from forks. Branches in
    the repo just create confusion, for instance, they appear whenever someone
    clones the repository.

54. **Related to the previous point, make pull request reverts come from
    forks.** Right now the create a branch in the same repo, but it would be
    better if the branch were made in the person's fork.

55. **Allow me to enable branch protection by default for new repos.**

56. **Allow me to enable branch protection by default on new branches.** This
...
57. **Clicking a team name the settings should default to the "members" tab.**
    The non-open "discussions" feature is terrible, and you should either open
    it by default or get rid of it.

58. **Suggest people to add push access to.** I don't necessarily mean
...
59. **Stop trying to make the profiles look "cute" with randomly highlighted
    ...
    unintended consequences to changes to it.

60. **Explain what the axes actually mean in the new "activity overview".**
    I'm referring to
...
61. **Allow us to hide the "source code (zip)" and "source code (tar.gz)"
    files in a release.** We upload our actual release files (generated by
    `setup.py`) to the GitHub release. We want people to download those, not
    snapshots of the repo.

## Miscellaneous

62. **A more official way to request fixes to these cuts.** I often ask on
...
63. **The repository search function doesn't support full partial matches.**
...
64. **Show commit history as a graph.** Like `git log --graph`. This would go
...
65. **Bring back the old "fork" UI.** The one that just had icons for all the
